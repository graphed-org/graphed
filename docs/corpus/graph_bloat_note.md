...
execution) came to dominate wall time. This note quantifies the problem.
...
1. **Per-operator recording.** A single ADL query is ~20–60 operators (see the operations
...
graph was still large enough that the Python interpreter walking it was itself a cost. The
`master` (virtual-array) branch escapes the blow-up by giving up the schedulable graph entirely.

## Order-of-magnitude estimate from the fixtures

Counting operators in the runnable fixtures. This is a proxy: the fixtures count awkward-level
operators, not the dask-awkward low-level layer counts, which have not been measured here.
...
## What graphed does instead

`graphed` reduces the graph **incrementally as you build it**, so the un-reduced O(10⁴) graph
never exists. What you hand to a runner is a concise stage-graph whose node count scales with the
number of stages in your analysis, **not with the systematics count** — and reduction time is held
to linear growth in graph size, so the optimizer can never again become the thing you wait on.
