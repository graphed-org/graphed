...
* `DeclaringSource` — `PlainSource` plus `projected_columns`, counting its calls, keeping the exact
  ...
  answer a tuple. `max_calls` turns any further call into an `AssertionError`.
...
The suite COLLECTS with zero errors (11 tests; every m58-new outcome is reached inside a test body)
...
* H1 ×3, H3 — `assert 0 == 1` on `source.calls`: no driver consults the hook today, so the counter
  never moves. (H3's later `pickle` leg is unreachable until it does.)
...
Four legs PASS on a pre-m58 tree — the declared regression controls, each a live instrument showing
the harness reaches the drivers at all:
...
The three H drivers were run with their plans' `columns` forced to `DECLARED`: all three still
produce the values asserted here, so no leg fails post-implementation for a fixture reason. E1/E3's
reports were measured against a replay whose `on_external` returns the recorded form's typetracer.
