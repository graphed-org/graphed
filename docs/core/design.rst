...
``SequentialRunner`` takes one, and so do the thread and process pools and the dask and parsl
runners in ``graphed-executors``. Every runner that takes a ``control`` honours it the same way:
