...
Internal requirements record: it is excluded from the rendered documentation (see
`exclude_patterns` in `docs/conf.py`) and written for maintainers, not readers. What the reference
analyses cover, in user terms, is `docs/corpus/design.rst`.

Each entry names an operation/pattern the `graphed` frontend must support, the corpus analysis it
is exercised by, and the **milestone** that delivered it. "Extract, do not invent": every row
below is exercised by a runnable fixture in `tests/_corpus/graphed_corpus/`, which
`tests/frozen/corpus/m05/test_catalog.py` keeps in lock-step with this file.
...
- **ADL queries 1–8** — `analyses/adl.py` → `tests/_corpus/references/adl_q{1..8}.json`. The graded ladder
...
- **AGC ttbar slice** — `analyses/systematics.py::ttbar_region`, regions {4j1b, 4j2b} × variations
  {nominal, jes_up, jes_down, btag_up, btag_down} → `tests/_corpus/references/ttbar_*.json`. Drives M7/M9.
- **TTGamma slice** — `analyses/systematics.py::ttgamma_region`, variations {nominal, jes_±, pho_±}
  → `tests/_corpus/references/ttgamma_*.json`.
...
- A `graphed.vary` fixture. The variation families here are written out by hand, one plain-awkward
  call per variation, which is what keeps them an independent reference for the graph axis.
