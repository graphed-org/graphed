...
def test_nested_stacked_diamonds_reduce_linearly() -> None:
    for d in (1, 8, 64, 256):
        ...
        _reduced, report = s.reduce(outputs=[v])
...
def test_topology_reduction_is_deterministic() -> None:
    def build() -> str:
        ...
        return s.reduce(outputs=[out])[0].to_dot()

    assert build() == build()


def _diamond_chain(nodes: int) -> tuple[gc.GraphStore, int]:
...
def _star(nodes: int) -> tuple[gc.GraphStore, int]:
    ...
    out = s.add_op("add", leaves)
    return s, out


def _best_reduce_time(store: gc.GraphStore, out: int, repeats: int = 5) -> float:
    best = float("inf")
